package ba.entries.entity.npc.warfare;

import ba.entries.entity.npc.warfare.trait.WardenTrait;
import minecraft.entity.EntityRendererType;
import minecraft.entity.Humanoid;
import minecraft.entity.mob.BaseMob;
import minecraft.extended.entity.GeneralHumanoidMob;
import net.minecraft.world.entity.EntityType;
import net.minecraft.world.level.Level;
import net.neoforged.neoforge.registries.DeferredHolder;

/**
 * 典狱长
 */
public class HumanWarden extends GeneralHumanoidMob {
	public static final String ID = "ba:npc_human_warden";

	public static final DeferredHolder<EntityType<?>, EntityType<HumanWarden>> TYPE = newType(HumanWarden.class, Humanoid.HUMANOID_WIDTH, Humanoid.HUMANOID_HEIGHT, ID, WardenTrait.ATTRIBUTES);

	public HumanWarden(EntityType<BaseMob> entityType, EntityRendererType<GeneralHumanoidModelInfo> rendererType, Level level) {
		super(entityType, rendererType, level);
		this.addTrait(new WardenTrait());
	}
}