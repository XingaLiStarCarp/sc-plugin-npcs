...
import minecraft.datagen.EntryHolder;
...
public @interface RegistryEntry {
	...
	public static class RegistryType {
		...
		public static boolean isEntryHolder(Field f, Class<?> genericType) {
			return reflection.is(f, EntryHolder.class) && reflection.is(reflection.first_generic_class(f), genericType);
		}
		...
		public static boolean isHolder(Field f, Class<?> genericType) {
			return reflection.is(f, Holder.class) && reflection.generic_start_with(f, genericType);
		}
	...
	public static class RuntimeEntryHolderRegister {
		...
		public static final void registerEntryHoldersToRegistry(Class<?> registryClass) {
			reflection.class_operation.walk_fields(registryClass, RegistryEntry.class, (Field f, boolean isStatic, Object value, RegistryEntry annotation) -> {
				if (isStatic) {
	...
	public static class RegistriesProvider extends DatapackBuiltinEntriesProvider {
		...
		private static final void setBootstrapContexts(EntryHolder<?> datagenHolder, BootstrapContext<?> context) {
			unsafe.write_member(datagenHolder, "bootstrapContext", context);
		}
			...
			reflection.class_operation.walk_fields(registryClass, RegistryEntry.class, (Field f, boolean isStatic, Object value, RegistryEntry annotation) -> {
				if (isStatic) {
					if (annotation.reg_stage() == Stage.DATA_GEN) {
						if (value == null) {
							Core.logWarn("@RegistryEntry(DATA_GEN) field {} is expected to have a non-null value.", f);
							return true;
						}
						if (RegistryType.isEntryHolder(f, registryType)) {
							...
							if (allowRegister) {
								datagenHolder.register();
								Core.logInfo("Datagen BootstrapContext<{}> registered [{}] entry [{}] with EntryHolder value={}",
										registryType.getSimpleName(),
										datagenHolder.registryKey().location(),
										datagenHolder.getKey().location(),
										datagenHolder.originalValue());
							} else {
								Core.logWarn("Datagen BootstrapContext<{}> EntryHolder entry [{}] with registry type [{}] is not allowed to register, check if this registry in RegistryEntry.RegistriesProvider.registryFieldFilter map.",
										registryType.getSimpleName(),
										datagenHolder.getKey().location(),
										datagenHolder.registryKey().location());
							}
							...
							if (allowRegister) {
								...
								Core.logInfo("Datagen BootstrapContext<{}> registered [{}] entry [{}] with Holder value={}",
										registryType.getSimpleName(),
										holder.getKey().registryKey().location(),
										holder.getKey().location(),
										holder.value());
							} else {
								Core.logWarn("Datagen BootstrapContext<{}> Holder entry [{}] with registry type [{}] is not allowed to register, check if this registry in RegistryEntry.RegistriesProvider.registryFieldFilter map.",
										registryType.getSimpleName(),
										holder.getKey().location(),
										holder.getKey().registryKey().location());
							}
					...
					Core.logWarn("@RegistryEntry field {} is expected to be declared as static.", f);
				return true;
			});
		}
