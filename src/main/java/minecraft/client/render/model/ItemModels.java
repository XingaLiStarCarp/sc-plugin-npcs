package minecraft.client.render.model;
...
import minecraft.mixins.internal.ItemRendererInternal;
...
public class ItemModels {
	...
	private static final ItemRendererInternal.Render.Callback modelReplaceFunc = (
		...
		if (map.containsKey(item_id)) {
			...
			if (item_model.itemType != null) {
				...
				currentOriginalRenderItem = (Item) unsafe.read_member_reference(itemStack, "item");// 储存当前的ItemStack.item
				unsafe.write_member(itemStack, "item", item_model.itemType);// 修改目标item成员
			}
		}
	};
	...
	private static final ItemRendererInternal.Render.Callback itemTypeRecoveryFunc = (
		...
		if (currentOriginalRenderItem != null) {
			unsafe.write_member(itemStack, "item", currentOriginalRenderItem);// ItemStack.item
