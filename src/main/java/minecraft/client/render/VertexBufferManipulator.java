...
public class VertexBufferManipulator {
	...
	private VertexBufferManipulator(MeshData mesh) {
		this.mesh = mesh;
		this.buffer_info = (MeshData.DrawState) unsafe.read_member_reference(mesh, "drawState");
		this.vertex_attributes = buffer_info.format();
		this.attribute_offsets = vertex_attributes.getOffsetsByElement();
		ByteBufferBuilder.Result vertexBuffer = (ByteBufferBuilder.Result) unsafe.read_member_reference(mesh, "vertexBuffer");
		ByteBufferBuilder builder = reflection.enclosing_class_instance(ByteBufferBuilder.class, vertexBuffer);
		long ptr = unsafe.read_member_long(builder, "pointer");
		long offset = unsafe.read_member_int(vertexBuffer, "offset");
	...
	public VertexBufferManipulator setPos(int vertexIdx, float x, float y, float z) {
		...
		unsafe.write(buffer + arrIdx, -x);
		unsafe.write(buffer + arrIdx + 4, -z);// 注意Minecraft坐标系的y轴实际上是OpenGL的z轴，y和z顺序需要互换，且正方向相反，所有分量需要取反
		unsafe.write(buffer + arrIdx + 8, -y);
		return this;
	}
	...
	public float[] getPos(int vertexIdx) {
		...
		pos[0] = -unsafe.read_float(buffer + arrIdx);
		pos[1] = -unsafe.read_float(buffer + arrIdx + 8);
		pos[2] = -unsafe.read_float(buffer + arrIdx + 4);
		return pos;
	}
	...
	public VertexBufferManipulator setUv(int vertexIdx, float u, float v) {
		...
		unsafe.write(buffer + arrIdx, u);
		unsafe.write(buffer + arrIdx + 4, v);
		return this;
	}
	...
	public float[] getUv(int vertexIdx) {
		...
		uv[0] = unsafe.read_float(buffer + arrIdx);
		uv[1] = unsafe.read_float(buffer + arrIdx + 4);
		return uv;
	}
	...
	public VertexBufferManipulator setColor(int vertexIdx, int r, int g, int b, int a) {
		...
		unsafe.write(buffer + arrIdx, (byte) r);
		unsafe.write(buffer + arrIdx + 1, (byte) g);
		unsafe.write(buffer + arrIdx + 2, (byte) b);
		unsafe.write(buffer + arrIdx + 3, (byte) a);
		return this;
	}
	...
	public int[] getColor(int vertexIdx) {
		int arrIdx = this.bufferIndex(vertexIdx, VertexFormatElement.COLOR);
		int[] color = new int[4];
		color[0] = unsafe.read_byte(buffer + arrIdx);
		color[1] = unsafe.read_byte(buffer + arrIdx + 1);
		color[2] = unsafe.read_byte(buffer + arrIdx + 2);
		color[3] = unsafe.read_byte(buffer + arrIdx + 3);
		return color;
	}
