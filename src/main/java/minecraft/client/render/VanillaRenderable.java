...
import minecraft.client.render.renderable.Renderable;
...
/**
 * OpenGL可渲染对象模型
 * MC的坐标系与右手坐标系不同，右手坐标系的顶点转MC需要：
 * 右手坐标系(x, y, z) -> MC坐标系(x, -y, z)
 * 由于不可见的三角形有背面剔除，右手坐标系中三角形的法线正方向与MC坐标系一致，因此无需更改绕顶点顺序
 */
@OnlyIn(Dist.CLIENT)
public class VanillaRenderable implements Renderable {
	...
	/**
	 * 添加三角形面，传入右手坐标系的三角形顶点坐标，且顶点逆时针为正方向
	 * 
	 * @param x1
	 * @param y1
	 * @param z1
	 ...
	 * @param b3
	 * @param a3
	 * @param invertNormal 是否反转三角形的正反面（法线方向）
	 * @return
	 */
	public VanillaRenderable addVertex(float x1, float y1, float z1, float u1, float v1, float r1, float g1, float b1, float a1,
		...
		if (invertNormal) {
			addVertex(x1, y1, z1, u1, v1, r1, g1, b1, a1);
			addVertex(x2, y2, z2, u2, v2, r2, g2, b2, a2);
			addVertex(x3, y3, z3, u3, v3, r3, g3, b3, a3);
		} else {
			addVertex(x1, y1, z1, u1, v1, r1, g1, b1, a1);
			addVertex(x3, y3, z3, u3, v3, r3, g3, b3, a3);
			addVertex(x2, y2, z2, u2, v2, r2, g2, b2, a2);
	...
	public VanillaRenderable addVertex(float x1, float y1, float z1, float r1, float g1, float b1, float a1,
	...
	public VanillaRenderable addVertex(float x1, float y1, float z1, float u1, float v1,
