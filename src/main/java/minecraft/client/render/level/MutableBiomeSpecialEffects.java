package minecraft.client.render.level;

import java.util.Optional;
import java.util.OptionalInt;

import math.interpolation.ColorLinearInterpolation;
import minecraft.core.registry.registries.client.DynamicRegistries;
import minecraft.event.LevelTickTrigger;
import minecraft.event.LevelTrigger;
import minecraft.resources.ResourceKeys;
import javabase.FieldRecoverable;
	...
	public final MutableBiomeSpecialEffects bindTo(String... biomesWithNamespace) {
		LevelTrigger.CLIENT_LEVEL_LOAD.addCallback(EventPriority.LOWEST, (LevelAccessor level) -> {
			for (String biomeKey : biomesWithNamespace) {
				...
				if (holder != null)
					unsafe.write_member(holder.value(), "specialEffects", effects);
			}
		});
		return this;
	}
