...
import minecraft.event.ServerLifecycleTrigger;
import minecraft.event.ServerTickTrigger;
import minecraft.event.ServerLifecycleTrigger.Operation;
import minecraft.event.ServerTickTrigger.TickOperation;
...
public class ServerInstance {
	...
	public static final void setServer(MinecraftServer server) {
		unsafe.write_static(ServerInstance.class, "server", server);
		ServerLevels.initLevels(server);
		// 如果不触发MappedRegistries类的初始化，那么就无法修改MappedRegistries.registryAccess的值，修改后会被初始化重新覆盖为null
		unsafe.ensure_class_initialized(MappedRegistryAccess.class);
		unsafe.write_static(MappedRegistryAccess.class, "serverRegistryAccess", server.registryAccess());
