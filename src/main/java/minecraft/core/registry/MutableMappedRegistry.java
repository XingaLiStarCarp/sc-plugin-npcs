...
import javabase.FieldReference;
import javabase.Recoverable;
import minecraft.resources.ResourceKeys;
...
public class MutableMappedRegistry<T> implements Recoverable<MutableMappedRegistry<T>> {
	...
	private MutableMappedRegistry(MappedRegistry<T> registry) {
		...
		key = (ResourceKey<? extends Registry<T>>) unsafe.read_reference(registry, "key");
		byId = (ObjectList<Holder.Reference<T>>) unsafe.read_reference(registry, "byId");
		toId = (Reference2IntMap<T>) unsafe.read_reference(registry, "toId");
		byLocation = (Map<ResourceLocation, Holder.Reference<T>>) unsafe.read_reference(registry, "byLocation");
		byKey = (Map<ResourceKey<T>, Holder.Reference<T>>) unsafe.read_reference(registry, "byKey");
		byValue = (Map<T, Holder.Reference<T>>) unsafe.read_reference(registry, "byValue");
		registrationInfos = (Map<ResourceKey<T>, RegistrationInfo>) unsafe.read_reference(registry, "registrationInfos");
		tags = (Map<TagKey<T>, Named<T>>) unsafe.read_reference(registry, "tags");
		unregisteredIntrusiveHolders = (Map<T, Holder.Reference<T>>) unsafe.read_reference(registry, "unregisteredIntrusiveHolders");
		lookup = (RegistryLookup<T>) unsafe.read_reference(registry, "lookup");
		tagAdditionLock = unsafe.read_reference(registry, "tagAdditionLock");
		this.asPrimary();
	}
	...
	public final MutableMappedRegistry<T> redirect() {
		unsafe.write(mappedRegistry, "key", key);
		unsafe.write(mappedRegistry, "byId", byId);
		unsafe.write(mappedRegistry, "toId", toId);
		unsafe.write(mappedRegistry, "byLocation", byLocation);
		unsafe.write(mappedRegistry, "byKey", byKey);
		unsafe.write(mappedRegistry, "byValue", byValue);
		unsafe.write(mappedRegistry, "registrationInfos", registrationInfos);
		unsafe.write(mappedRegistry, "tags", tags);
		unsafe.write(mappedRegistry, "unregisteredIntrusiveHolders", unregisteredIntrusiveHolders);
		return this;
	}

	@Override
	public MutableMappedRegistry<T> recovery() {
		unsafe.write(mappedRegistry, "key", pkey);
		unsafe.write(mappedRegistry, "byId", pbyId);
		unsafe.write(mappedRegistry, "toId", ptoId);
		unsafe.write(mappedRegistry, "byLocation", pbyLocation);
		unsafe.write(mappedRegistry, "byKey", pbyKey);
		unsafe.write(mappedRegistry, "byValue", pbyValue);
		unsafe.write(mappedRegistry, "registrationInfos", pregistrationInfos);
		unsafe.write(mappedRegistry, "tags", ptags);
		unsafe.write(mappedRegistry, "unregisteredIntrusiveHolders", punregisteredIntrusiveHolders);
		return this;
	}
