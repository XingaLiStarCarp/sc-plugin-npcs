...
public class MappedRegistryAccess {
	...
	public static final void initializeRegistryFields(Class<?> target, RegistryAccess.Frozen registryAccess) {
		reflection.class_operation.walk_fields(target, MappedRegistry.class, (Field f, boolean isStatic, MappedRegistry registry) -> {
			MappedRegistry actual_registry = null;
			// 判断目标是否是静态字段，以及目标字段是否可以赋值给MappedRegistry（即目标字段是否是MappedRegistry类或其子类）
			if (isStatic) {
				try {
					actual_registry = getUnfrozenRegistry(registryAccess, (ResourceKey) unsafe.read_reference(Registries.class, f.getName()));
				} catch (Exception ex) {
				...
				unsafe.write(target, f.getName(), actual_registry);
			}
			return true;
		});
	}
	...
	public static <T> boolean isFrozen(Registry<T> registry) {
		return (boolean) unsafe.read_reference(registry, "frozen");
	}
	...
	public static <T> MappedRegistry<T> unfreezeRegistry(Registry<T> registry) {
		unsafe.write(registry, "frozen", false);// 解冻注册表
		unsafe.write(registry, "unregisteredIntrusiveHolders", new IdentityHashMap<>());// 将尚未注册的Holder清空，防止抛出错误
