...
import minecraft.resources.TagKeys;
...
public class HolderSets {
	...
	static {
		HolderSet$Named = symbols.find_constructor(HolderSet.Named.class, HolderOwner.class, TagKey.class);
	}
	...
	public static final <T> HolderSet.Named<T> setContents(HolderSet.Named<T> holderSet, List<Holder<T>> contents) {
		unsafe.write_member(holderSet, "contents", contents);
		return holderSet;
	}

	@SuppressWarnings("unchecked")
	public static final <T> List<Holder<T>> getContents(HolderSet.Named<T> holderSet) {
		return (List<Holder<T>>) unsafe.read_reference(holderSet, "contents");
	}

	private static final MethodHandle HolderSet$Named_bind;

	static {
		HolderSet$Named_bind = symbols.find_method(HolderSet.Named.class, "bind", List.class);
	}
