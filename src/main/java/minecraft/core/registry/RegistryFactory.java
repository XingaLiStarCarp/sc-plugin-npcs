...
public class RegistryFactory {
	...
	public static <T> DeferredRegister<T> deferredRegister(ResourceKey<? extends Registry<T>> registryKey, String modId) {
		return (DeferredRegister<T>) registries.computeIfAbsent(registryKey, modId, (ResourceKey<?> resKey, String mod_id) -> {
	...
	private static Field DeferredRegister_registeredEventBus;

	static {
		DeferredRegister_registeredEventBus = reflection.find_declared_field(DeferredRegister.class, "registeredEventBus");
	}

	public static void register(DeferredRegister<?> register, IEventBus modBus) {
		if (!(boolean) unsafe.read_member_bool(register, DeferredRegister_registeredEventBus))
	...
	public static void registerStatic(Class<?> registryFactory, IEventBus modBus) {
		reflection.class_operation.walk_fields(registryFactory, (Field f, boolean isStatic, Object value) -> {
	...
	public static void registerAll(IEventBus modBus) {
		for (DeferredRegister<?> register : registries.values()) {
			Core.logInfo("Start to register registry '{}'", register.getRegistryKey().location());
			register(register, modBus);
	...
	public static void registerAll() {
		Core.logInfo("RegistryFactory start to auto register, total DeferredRegister num: {}.", registries.size());
	...
	public static <R> Map<DeferredHolder<R, ? extends R>, Supplier<? extends R>> deferredRegisterEntries(DeferredRegister<R> register) {
		return (Map<DeferredHolder<R, ? extends R>, Supplier<? extends R>>) unsafe.read_reference(register, "entries");
	}
}
