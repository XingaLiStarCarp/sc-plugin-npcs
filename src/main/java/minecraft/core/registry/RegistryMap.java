...
import minecraft.item.ExtCreativeTab;
 ...
 * @param <_RegTp>
 */
public class RegistryMap<_RegTp> {
	/**
	...
	protected final ResourceKey<? extends Registry<_RegTp>> registryKey;
	protected final HashMap<String, DeferredHolder<_RegTp, ? extends _RegTp>> entriesMap = new HashMap<>();
	protected final HashMap<String, DeferredRegister<_RegTp>> deferredRegisters = new HashMap<>();

	private RegistryMap(ResourceKey<? extends Registry<_RegTp>> registry_key) {
		this.registryKey = registry_key;
	}

	/**
	 * 保证每个注册类型只有单一RegistryMap<_RegTp>实例
	 */
	private static final HashMap<ResourceKey<? extends Registry<?>>, RegistryMap<?>> RegistryMaps = new HashMap<>();

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static final <_RegTp> RegistryMap<_RegTp> of(ResourceKey<? extends Registry<_RegTp>> registry_key) {
		if (Registries.ITEM.equals(registry_key))
			return (RegistryMap<_RegTp>) RegistryMaps.computeIfAbsent(registry_key, (ResourceKey<? extends Registry<?>> key) -> new RegistryMap.ItemMap());
		else if (Registries.BLOCK.equals(registry_key))
			return (RegistryMap<_RegTp>) RegistryMaps.computeIfAbsent(registry_key, (ResourceKey<? extends Registry<?>> key) -> new RegistryMap.BlockMap());
		else
			return (RegistryMap<_RegTp>) RegistryMaps.computeIfAbsent(registry_key, (ResourceKey<? extends Registry<?>> key) -> new RegistryMap(key));
	}

	public final DeferredRegister<_RegTp> getDeferredRegister(String namespace) {
	 ...
	 * @return
	 */
	public <_Entry extends _RegTp> DeferredHolder<_RegTp, _Entry> register(String name, Supplier<_Entry> sup, Object... args) {
		ResourceLocation loc = ResourceLocations.build(name);
		namespaces.add(loc.getNamespace());// 将当前注册的元素命名空间加入datagen命名空间集合
		DeferredHolder<_RegTp, _Entry> entry = (DeferredHolder<_RegTp, _Entry>) getDeferredRegister(loc.getNamespace()).register(loc.getPath(), sup);
		entriesMap.put(name, entry);
		this.applyExtraArgs(name, entry, args);
		return entry;
	}

	public <_ArgTp, _Entry extends _RegTp> DeferredHolder<_RegTp, _Entry> register(String name, Function<_ArgTp, _Entry> func, _ArgTp func_arg, Object... args) {
	 ...
	 * @param args
	 */
	protected <_Entry extends _RegTp> void applyExtraArgs(String name, DeferredHolder<_RegTp, _Entry> entry, Object... args) {

	}

	@SuppressWarnings("unchecked")
	public static final <_ArgTp> _ArgTp fetchArg(int idx, Class<_ArgTp> cls, Object... args) {
	...
	public DeferredHolder<_RegTp, ? extends _RegTp> get(String name) {
