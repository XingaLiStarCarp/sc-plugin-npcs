	...
	class Bootstrap {
		...
		private static void onRegister(RegisterEvent event) {
			...
			for (Class<?> BootstrapRegistriesClass : BootstrapRegistriesClasses)
				reflection.class_operation.walk_fields(BootstrapRegistriesClass, MappedRegistry.class, (Field f, boolean isStatic, MappedRegistry registry) -> {
					if (isStatic && RegistryWalker.getRegistryKeyType(f) == type) {
						unsafe.write(BootstrapRegistriesClass, f.getName(), reg);
					}
					return true;
				});
		}
