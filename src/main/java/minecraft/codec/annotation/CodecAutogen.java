...
import minecraft.core.registry.RegistryWalker;
...
public @interface CodecAutogen {
	 ...
	 * @return
	 */
	boolean warn_if_register_failed()
	...
	public static class CodecAutogenAttributes {
		private final Class<?> base_class;
		private final boolean null_if_empty;
		private final boolean register;
		private final boolean warn_if_register_failed;
		private final boolean include_base;

		private CodecAutogenAttributes(Class<?> base_class, boolean null_if_empty, boolean register, boolean warn_if_register_failed, boolean include_base) {
			this.base_class = base_class;
			this.null_if_empty = null_if_empty;
			this.register = register;
			this.warn_if_register_failed = warn_if_register_failed;
			this.include_base = include_base;
		}

		public final boolean hasAutoAttributes(Class<?> targetClass) {
			return reflection.is(targetClass, base_class);
		}

		public final boolean register(Class<?> targetClass) {
			return hasAutoAttributes(targetClass) ? register : false;
		}

		public final boolean null_if_empty(Class<?> targetClass) {
			return hasAutoAttributes(targetClass) ? null_if_empty : false;
		}

		public final boolean warn_if_register_failed(Class<?> targetClass) {
			return hasAutoAttributes(targetClass) ? warn_if_register_failed : false;
		}

		public final boolean include_base(Class<?> targetClass) {
			return hasAutoAttributes(targetClass) ? include_base : false;
		}

		public static final CodecAutogenAttributes of(Class<?> base_class, boolean null_if_empty, boolean register, boolean warn_if_register_failed, boolean include_base) {
			return new CodecAutogenAttributes(base_class, null_if_empty, register, warn_if_register_failed, include_base);
		}

		@skip_unwind
		public static final CodecAutogenAttributes of(boolean null_if_empty, boolean register, boolean warn_if_register_failed, boolean include_base) {
			return of(stack.get_caller_class(), null_if_empty, register, warn_if_register_failed, include_base);
		}

		/**
		 * 子类CODEC是否自动注册，如果标记为自动注册则无论CodecAutogen.register()值为多少均会注册
		 */
		private static final ArrayList<CodecAutogenAttributes> derivedAutoAttributes = new ArrayList<>();
		 ...
		 * @param targetClass
		 */
		public static final void markDerivedAutoAttributes(CodecAutogenAttributes attributes) {
			derivedAutoAttributes.add(attributes);
		}

		public static final boolean isMarkedAutoRegister(Class<?> targetClass) {
			for (CodecAutogenAttributes attributes : derivedAutoAttributes) {
				if (attributes.hasAutoAttributes(targetClass))
					return attributes.register;
			}
			return false;
		}

		public static final boolean isMarkedAutoNullIfEmpty(Class<?> targetClass) {
			for (CodecAutogenAttributes attributes : derivedAutoAttributes) {
				if (attributes.hasAutoAttributes(targetClass))
					return attributes.null_if_empty;
			}
			return false;
		}

		public static final boolean isMarkedAutoWarnIfRegisterFailed(Class<?> targetClass) {
			for (CodecAutogenAttributes attributes : derivedAutoAttributes) {
				if (attributes.hasAutoAttributes(targetClass))
					return attributes.warn_if_register_failed;
			}
			return false;
		}

		public static final boolean isMarkedAutoIncludeBase(Class<?> targetClass) {
			for (CodecAutogenAttributes attributes : derivedAutoAttributes) {
				if (attributes.hasAutoAttributes(targetClass))
					return attributes.include_base;
			}
			return false;
		}
	}
	...
	public static class CodecGenerator {
		...
		private static final structs.instant_iterate_list<Class<?>> codecClasses = new structs.instant_iterate_list<Class<?>>();
		...
		static {
			create = symbols.find_static_method(RecordCodecBuilder.class, "create", Codec.class, Function.class);
			mapCodec = symbols.find_static_method(RecordCodecBuilder.class, "mapCodec", MapCodec.class, Function.class);
		}
		...
		public static final Object generate(Class<?> targetClass, MethodHandle buildMethod, boolean null_if_empty, boolean include_base) {
			...
			MethodHandle ctor = codecCtors.computeIfAbsent(targetClass, (Class<?> target) -> symbols.find_constructor(target, __types));// 寻找目标类的构造函数，如果没有在forCodec()时手动指定构造函数参数类型，则依据收集到的字段声明类型严格匹配寻找构造函数
			if (ctor == null)
				throw new IllegalArgumentException("Cannot find constructor when generating CODEC for " + targetClass + ", check the fields annotated with CodecEntry's order and type, make sure that both order and type are completely the same.\n" +
						"Expected constructor form: " + symbols.constructor_description(targetClass, __types) + ";");
			Object CODEC = null;
			switch (entries.size()) {
			case 0:
		...
		public static final DeferredHolder generateAndRegisterHolder(String namespace, String codecName, Class<?> target, MethodHandle buildMethod, Class<?> registryType, boolean null_if_empty, boolean include_base) {
			...
			if (CODEC == null)
				return null;
			java_type.wrapper<DeferredHolder> holderWrapper = java_type.wrapper.wrap();
		...
		private static final Object generateAndRegister(String namespace, String codecName, Class<?> target, MethodHandle buildMethod, Class<?> registryType, boolean null_if_empty, boolean register, boolean warn_if_register_failed, boolean include_base) {
			Object CODEC = generate(target, buildMethod, null_if_empty || CodecAutogenAttributes.isMarkedAutoNullIfEmpty(target), include_base || CodecAutogenAttributes.isMarkedAutoIncludeBase(target));
			if (CODEC == null)
				return null;
			Core.logInfo("Generated CODEC for " + target + " -> " + CODEC);
			if (register || CodecAutogenAttributes.isMarkedAutoRegister(target)) {
				...
				java_type.wrapper<Boolean> registered = java_type.wrapper.wrap(false);
				RegistryWalker.walkMapCodecRegistries((Field f, ResourceKey registryKey, Class<?> codecType) -> {
					if (reflection.is(registryType, codecType)) {
						// 当注册表MapCodec的泛型参数和传入的registryType匹配时注册
						// 只向DeferredRegister注册，不需要向BootstrapContext注册。datagen时会使用DeferredRegister注册的CODEC进行序列化
						RegistryFactory.deferredRegister(registryKey, namespace).register(codecName, () -> CODEC);
						registered.value = true;
						Core.logInfo("Registered CODEC {} in registry [{}].", codecName, registryKey.location());
					}
					return true;
				});
				if (!registered.value && (warn_if_register_failed || CodecAutogenAttributes.isMarkedAutoWarnIfRegisterFailed(target)))
					Core.logWarn("CODEC {} was not registered, the registry for MapCodec<{}> was not found. Maybe it isn't a MapCodec registry?", codecName, registryType.getName());
			}
			return CODEC;
		}
		...
		private static final void generateAndRegisterCodecs(Class<?> codecClass) {
			reflection.class_operation.walk_fields(codecClass, CodecAutogen.class, (Field f, boolean isStatic, Object value, CodecAutogen annotation) -> {
				if (isStatic) {
					...
					Object CODEC = generateAndRegister(annotation.namespace(), registerName, codecClass, internalBuildMethod, reflection.first_generic_class(f), annotation.null_if_empty(), annotation.register(), annotation.warn_if_register_failed(), annotation.include_base());
					if (isKeyDispatchDataCodec)
						CODEC = KeyDispatchDataCodec.of((MapCodec) CODEC);
					unsafe.write(codecClass, f, CODEC);
				}
				return true;
			});
		}
		...
		public static <_Tp> void testCodec(MapCodec<_Tp> codec, _Tp obj) {
			try {
				DynamicOps<JsonElement> ops = JsonOps.INSTANCE;
				DataResult<JsonElement> result = codec.codec().encodeStart(ops, obj);
				result.resultOrPartial(error -> {
					Core.logError("codec serialize error: {}", error);
				}).ifPresent(json -> {
					Core.logInfo("codec serialize success: {}", json.toString());
				});
			} catch (Exception ex) {
				Core.logError("codec serialize exception: {}", Core.throwableString(ex));
			}
		}

		public static <_Tp> void testCodec(KeyDispatchDataCodec<_Tp> codec, _Tp obj) {
			testCodec(codec.codec(), obj);
		}
		...
		public static final void forCodec(Class<?> targetClass, Class<?>... ctorTypes) {
			...
			if (CodecTarget.TypeChecker.check(targetClass, ctorTypes) && ctorTypes.length != 0) {// 通过了类型检查，且手动传入了参数则寻找构造函数，否则就遍历字段时寻找
				MethodHandle ctor = symbols.find_constructor(targetClass, ctorTypes);
				if (ctor == null)
		...
		@skip_unwind
		public static final void callerCodec(Class<?>... ctorTypes) {
			forCodec(stack.get_caller_class(), ctorTypes);
		}
	}
}
