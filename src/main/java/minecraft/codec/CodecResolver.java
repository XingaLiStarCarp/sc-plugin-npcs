 ...
 */
@FunctionalInterface
public interface CodecResolver<_T> {
	public CodecInfo resolve(Type fieldType, CodecType codecType, _T param);
	...
	public static CodecResolver<CodecEntry> findInRawClass(Class<?> targetClass, Object default_codec_if_not_exist) {
		return (Type fieldType, CodecType codecType, CodecEntry param) -> {
			...
			if (result == null)
				Core.logError("find Codec for target entry type '{}' failed", targetClass);
			if (param == null)
				return CodecInfo.of(result);
			else
				return CodecInfo.of(result, param.type());
		};
	}
	...
	public static enum CodecType {
		...
		public Object accessOrDefault(Class<?> targetClass, Object default_codec) {
			return unsafe.read_reference(targetClass, getField(targetClass), default_codec);
		}
	...
	public static final class CodecInfo {
		...
		private CodecInfo(Object codec, RequirementType type) {
			if (codec == null)
				throw new IllegalStateException("CodecResolver.Info cannot be create from null CODEC.");
			this.codec = codec;
			this.type = type;
		}

		private CodecInfo(Object codec) {
		...
		public static final CodecInfo of(Object codec, RequirementType type) {
			return new CodecInfo(codec, type);
		}

		public static final CodecInfo of(Object codec) {
			return new CodecInfo(codec);
		}
	}
}
