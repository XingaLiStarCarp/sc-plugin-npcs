...
import minecraft.codec.CodecResolver.CodecType;
import minecraft.codec.annotation.CodecEntry;
import minecraft.codec.annotation.DoubleRange;
import minecraft.codec.annotation.FloatRange;
import minecraft.codec.annotation.IntRange;
import minecraft.codec.annotation.ListSize;
import minecraft.codec.annotation.StringLength;
import minecraft.core.registry.Holders;
...
public class Codecs {
	...
	static {
		...
		CodecResolver<CodecEntry> INT = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			...
			if (ir == null)
				return CodecResolver.CodecInfo.of(Codec.INT);
			else
				return CodecResolver.CodecInfo.of(Codec.intRange(ir.min(), ir.max()));
		};
		...
		CodecResolver<CodecEntry> FLOAT = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			...
			if (fr == null)
				return CodecResolver.CodecInfo.of(Codec.FLOAT);
			else
				return CodecResolver.CodecInfo.of(Codec.floatRange(fr.min(), fr.max()));
		};
		...
		CodecResolver<CodecEntry> DOUBLE = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			...
			if (dr == null)
				return CodecResolver.CodecInfo.of(Codec.DOUBLE);
			else
				return CodecResolver.CodecInfo.of(Codec.doubleRange(dr.min(), dr.max()));
		};
		...
		CodecResolver<CodecEntry> STRING = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			...
			if (sl == null)
				return CodecResolver.CodecInfo.of(Codec.STRING);
			else
				return CodecResolver.CodecInfo.of(Codec.string(sl.min(), sl.max()));
		};
		...
		CodecResolver<Object> HOLDER = (Type fieldType, CodecType codecType, Object param) -> {
			Class<?> holderType = Holders.getHolderType(fieldType);// Holder<holderType [<reflection.generic_type(type, 0)>] >
			Codec<?> codec = getCodec(holderType, reflection.generic_type(fieldType, 0), CodecType.HOLDER, param).codec();
			if (codec == null)
				throw new IllegalArgumentException("No CODEC found for type " + holderType + " in Holder entry.");
			return CodecResolver.CodecInfo.of(codec);
		};
		...
		CodecResolver<CodecEntry> LIST = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			Class<?> listType = reflection.first_generic_class(fieldType);
			Codec<?> codec = getCodec(listType, reflection.generic_type(fieldType, 0), codecType, param).codec();
			if (codec == null)
			...
			if (ls == null)
				return CodecResolver.CodecInfo.of(codec.listOf());
			else
				return CodecResolver.CodecInfo.of(codec.listOf(ls.min(), ls.max()));
		};
		...
		CodecResolver<CodecEntry> MAP = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			Class<?> keyType = reflection.generic_classes(fieldType)[0].type();
			Codec<?> keyCodec = getCodec(keyType, reflection.generic_type(fieldType, 0), codecType, param).codec();
			if (keyCodec == null)
				throw new IllegalArgumentException("No CODEC found for key type " + keyType + " in Map entry.");
			Class<?> valueType = reflection.generic_classes(fieldType)[0].type();
			Codec<?> valueCodec = getCodec(valueType, reflection.generic_type(fieldType, 1), codecType, param).codec();
			if (valueCodec == null)
				throw new IllegalArgumentException("No CODEC found for value type " + valueType + " in Map entry.");
			return CodecResolver.CodecInfo.of(Codec.unboundedMap(keyCodec, valueCodec));
		};
		setCodec(Map.class, MAP);
		// 可选值编解码器
		CodecResolver<CodecEntry> OPTIONAL_LONG = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			return CodecResolver.CodecInfo.of(Codec.LONG, CodecResolver.CodecInfo.RequirementType.OPTIONAL);
		};
		...
		CodecResolver<CodecEntry> OPTIONAL = (Type fieldType, CodecType codecType, CodecEntry param) -> {
			Class<?> optionalType = reflection.first_generic_class(fieldType);
			Codec<?> codec = getCodec(optionalType, reflection.generic_type(fieldType, 0), codecType, param).codec();
			if (codec == null)
				throw new IllegalArgumentException("No CODEC found for type " + optionalType + " in Optional entry.");
			return CodecResolver.CodecInfo.of(codec, CodecResolver.CodecInfo.RequirementType.OPTIONAL);
		};
	...
	public static final void setCodecByFieldName(Class<?> targetClass, String codecFieldName) {
		...
		CODEC_RESOLVERS.put(targetClass, (Type fieldType, CodecType codecType, Object param) -> CodecResolver.CodecInfo.of(unsafe.read_reference(targetClass, codecFieldName)));
	}
	...
	public static final void setCodec(Class<?> targetClass, Object codec) {
		CODEC_RESOLVERS.put(targetClass, (Type fieldType, CodecType codecType, Object param) -> CodecResolver.CodecInfo.of(codec));
	}
	...
	public static final CodecResolver.CodecInfo getCodec(Class<?> targetClass, Type fieldType, CodecType codecType, Codec<?> default_codec_if_not_exist, Object param) {
	...
	public static final CodecResolver.CodecInfo getCodec(Class<?> targetClass, Type fieldType, CodecType codecType, boolean empty_codec_if_not_exist, Object param) {
		return getCodec(targetClass, fieldType, codecType, empty_codec_if_not_exist ? Codecs.emptyCodec(targetClass) : null, param);
	}

	public static final CodecResolver.CodecInfo getCodec(Class<?> targetClass, Type fieldType, CodecType codecType, Object param) {
		return getCodec(targetClass, fieldType, codecType, null, param);
	}

	public static final CodecResolver.CodecInfo getCodec(Class<?> targetClass, CodecType codecType) {
	...
	public static final <O> MapCodec<O> emptyMapCodec(Class<O> target) {
		return new MapCodec<O>() {
			...
			public <T> DataResult<O> decode(DynamicOps<T> ops, MapLike<T> input) {
				try {
					return (DataResult<O>) DataResult.success((O) symbols.find_constructor(target).invoke());
				} catch (Throwable ex) {
					ex.printStackTrace();
				}
	...
	public static final <O> MapCodec<O> emptyMapCodec() {
		Class<O> caller = (Class<O>) stack.get_caller_class();
