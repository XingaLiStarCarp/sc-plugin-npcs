...
public class CodecWalker {
	...
	public static final void walkMapCodecs(Class<?> target, GenericMapCodecOperation op) {
		reflection.class_operation.walk_fields(target, MapCodec.class, (Field f, boolean isStatic, Class<?> codecType, MapCodec codec) -> {
			if (isStatic) {
	...
	public static final void walkCodecs(Class<?> target, GenericCodecOperation op) {
		reflection.class_operation.walk_fields(target, Codec.class, (Field f, boolean isStatic, Class<?> codecType, Codec codec) -> {
			if (isStatic) {
				return op.operate(f, codec, codecType);
			}
			return true;
	...
	public static final void walkKeyDispatchDataCodecs(Class<?> target, GenericKeyDispatchDataCodecOperation op) {
		reflection.class_operation.walk_fields(target, KeyDispatchDataCodec.class, (Field f, boolean isStatic, Class<?> codecType, KeyDispatchDataCodec codec) -> {
			if (isStatic) {
				return op.operate(f, codec, codecType);
			}
			return true;
