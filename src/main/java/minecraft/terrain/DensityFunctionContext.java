 ...
 * 仅数据生成阶段有效。
 */
public class DensityFunctionContext {
	...
	private DensityFunctionContext(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
		this.densityFunctions = densityFunctions;
		this.noiseParameters = noiseParameters;
	}

	private DensityFunctionContext(BootstrapContext<?> context) {
	...
	public Holder<NormalNoise.NoiseParameters> staticParam(Class<?> noiseParameters, String noiseParametersKey) {
		return param((ResourceKey<NormalNoise.NoiseParameters>) unsafe.read_reference(noiseParameters, noiseParametersKey));
	}
	...
	public DensityFunction staticFunc(Class<?> staticDensityFunctionsClass, String densityFunctionsKey) {
		return func((ResourceKey<DensityFunction>) unsafe.read_reference(staticDensityFunctionsClass, densityFunctionsKey));
	}
	...
	public static DensityFunctionContext of(HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParameters) {
		if (densityFunctions != null && noiseParameters != null)
			return new DensityFunctionContext(densityFunctions, noiseParameters);
		return null;
	}

	public static DensityFunctionContext of(BootstrapContext<?> context) {
		if (context != null)
			return new DensityFunctionContext(context);
		return null;
	}
